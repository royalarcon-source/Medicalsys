...
export class AtencionServicioService {
  ...
  static async listarPorConsulta(
    idConsulta: number,
    ...
    if (authUser?.rol === "PACIENTE") {
      ...
      const pacienteIdUsuario = consulta?.historia?.paciente?.usuario?.idUsuario;
      if (!consulta || pacienteIdUsuario !== authUser.idUsuario) {
        throw { status: 403, message: "Acceso denegado: solo puede consultar los servicios de sus propias consultas." };
      }
    }
  ...
  static async listarPorPaciente(
    ...
    estado?: string,
    authUser?: { idUsuario: number; rol: string }
  ): Promise<AtencionServicio[]> {
    if (authUser?.rol === "PACIENTE") {
      const pacientePropio = await PacienteRepository.buscarPorUsuario(authUser.idUsuario);
      if (!pacientePropio || pacientePropio.idPaciente !== idPaciente) {
        throw { status: 403, message: "Acceso denegado: solo puede consultar sus propios servicios." };
      }
    }
