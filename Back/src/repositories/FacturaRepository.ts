...
export const FacturaRepository = AppDataSource.getRepository(Factura).extend({
  ...
  // Debe invocarse con el `manager` de la transacción en la que se insertará la factura:
  // el advisory lock es de alcance transaccional (`pg_advisory_xact_lock`) y se libera
  // automáticamente al hacer commit/rollback, serializando la generación del correlativo
  // entre emisiones concurrentes y evitando que dos facturas terminen con el mismo número.
  async generarSiguienteNumero(manager: EntityManager): Promise<string> {
    const anioActual = new Date().getFullYear();
    const prefijo = `FAC-${anioActual}-`;

    await manager.query("SELECT pg_advisory_xact_lock(hashtext($1))", [prefijo]);

    const ultimaFactura = await manager
