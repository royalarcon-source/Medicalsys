...
- ✅ **Diagrama de Componentes**: Representa la arquitectura en capas general de todo el sistema (Frontend React, Express Router, Middlewares, Controladores, Servicios de Negocio, Cifrado, ORM TypeORM, PostgreSQL y Cloudinary).
- ✅ **Modelado UML**: Diagrama de clases general con todas las entidades del dominio de MedicalSys (`Usuario`, `Rol`, `Paciente`, `Medico`, `Especialidad`, `HistoriaClinica`, `Consulta`, `Diagnostico`, `Tratamiento`, `Cita`, `Consultorio`, `Factura`, `DetalleFactura`, `Consentimiento`, `Documento`, `AuditLog`, `Anuncio` y `Notificacion`).
- ✅ **Diagrama de Tiempos / Secuencia**: Ilustra el ciclo de vida general de peticiones $T_0 \to T_5$ aplicando el flujo de seguridad, procesamiento de negocio, cifrado de datos, persistencia y auditoría del sistema.
