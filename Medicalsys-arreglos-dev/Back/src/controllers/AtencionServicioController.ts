...
export const AtencionServicioController = {
  ...
  async listarPorConsulta(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const resultado = await AtencionServicioService.listarPorConsulta(idConsulta);
      return res.status(200).json(resultado);
    } catch (error) {
      return next(error);
    }
  ...
  async listarPorPaciente(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const resultado = await AtencionServicioService.listarPorPaciente(idPaciente, estado);
      return res.status(200).json(resultado);
    } catch (error) {
      return next(error);
    }
