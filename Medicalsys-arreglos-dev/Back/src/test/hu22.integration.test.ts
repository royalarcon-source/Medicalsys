...
const BASE_URL = process.env.HU22_BASE_URL ?? "http://localhost:3000/api";
const JWT_SECRET = process.env.JWT_SECRET;
const RUN_ID = Date.now().toString();
const REPORT_PATH = path.resolve(__dirname, "../../../docs/T7_HU22_Reporte_Pruebas.md");
...
async function crearDatos(): Promise<void> {
  ...
  for (const nombre of ["ADMINISTRADOR", "MEDICO", "RECEPCIONISTA", "PACIENTE"]) {
    ...
    if (!rol) {
      rol = await rolRepo.save(rolRepo.create({ nombre, descripcion: `Rol temporal HU22 (${RUN_ID})` }));
      rolesCreados.push(rol);
    }
    roles.set(nombre, rol);
  }

  const passwordHash = await bcrypt.hash("HU22-test-only", 4);
  const perfiles = [
    ["AdminHu22", "Test", "ADMINISTRADOR"],
    ["MedicoHu22", "Test", "MEDICO"],
    ["RecepcionistaHu22", "Test", "RECEPCIONISTA"],
    ["PacienteHu22", "Test", "PACIENTE"],
  ] as const;
  ...
  for (const [nombres, apellidos, rolNombre] of perfiles) {
    const usuario = usuarioRepo.create({
      ...
      nombres,
      apellidos,
      email: `hu22-${nombres.toLowerCase()}-${RUN_ID}@test.invalid`,
      passwordHash,
      telefono: "70000000",
      activo: true,
    });
  ...
  const medico = medicoRepo.create({
    usuario: usuarios[1],
    numeroColegiatura: `HU22-COL-${RUN_ID}`,
    activo: true,
  });
  ...
  const paciente = pacienteRepo.create({
    usuario: usuarios[3],
    documentoIdentidad: `CI-HU22-${RUN_ID}`,
    fechaNacimiento: new Date("1987-05-15"),
    sexo: "M",
    direccion: "Avenida prueba 456",
    contactoEmergencia: "Contacto HU22",
    telefonoEmergencia: "72222222",
  });
  ...
  const historia = historiaRepo.create({
    paciente,
    observaciones: "Historia creada para pruebas de tratamiento HU22",
  });
  ...
  consultaPrueba = await consultaRepo.save(
    consultaRepo.create({
      ...
      cita: null,
      consultorio: null,
      motivo: "Tratamiento por seguimiento",
...
async function ejecutar(): Promise<void> {
  ...
  try {
    ...
    await comprobar("ST22.1", "Registro exitoso con rol MEDICO", async () => {
      const response = await api.post(
        "/tratamientos",
        {
          idConsulta: consultaPrueba!.idConsulta,
          descripcion: "Amoxicilina 500mg",
          indicaciones: "Tomar cada 12 horas por 5 días",
      ...
      tratamientoCreadoId = response.data.tratamiento.idTratamiento;
      return datosRespuesta(response);
    });

    await comprobar("ST22.2", "Consulta inexistente devuelve 404", async () => {
      const response = await api.post(
        "/tratamientos",
        {
          idConsulta: 999999,
          descripcion: "Tratamiento para consulta inexistente",
          fechaInicio: "2026-09-01",
          fechaFin: "2026-09-03",
        },
        { headers: medico }
    ...
    await comprobar("ST22.3", "Paciente sin permiso recibe 403", async () => {
      const response = await api.post(
        "/tratamientos",
    ...
    await comprobar("ST22.4", "Recepcionista sin permiso recibe 403", async () => {
      const response = await api.post(
        "/tratamientos",
        { idConsulta: consultaPrueba!.idConsulta, descripcion: "Intento no autorizado" },
    ...
    await comprobar("ST22.5", "Validación de campos y coherencia de fechas devuelve 400", async () => {
      const response = await api.post(
        "/tratamientos",
        {
          idConsulta: consultaPrueba!.idConsulta,
    ...
    await comprobar("ST22.6", "Sin autenticación recibe 401", async () => {
      const response = await api.post("/tratamientos", {
        idConsulta: consultaPrueba!.idConsulta,
        descripcion: "Tratamiento sin token",
      });
      assert(response.status === 401, datosRespuesta(response));
      return datosRespuesta(response);
    });

    await comprobar("ST22.7", "Token inválido recibe 401", async () => {
      const response = await api.post(
        "/tratamientos",
        { idConsulta: consultaPrueba!.idConsulta, descripcion: "Tratamiento con token inválido" },
    ...
    await comprobar("ST22.8", "Acceso administrativo con token válido", async () => {
      const response = await api.get(`/tratamientos/${tratamientoCreadoId}`, { headers: admin });
    ...
    if (!conexionPostgres || /ECONNREFUSED|ENOTFOUND|Servidor no disponible/i.test(evidencia)) {
      resultados.push({ id: "SETUP", nombre: "Preparación del entorno HU-22", estado: "BLOQUEADA", evidencia });
    } else {
      resultados.push({ id: "SETUP", nombre: "Preparación del entorno HU-22", estado: "FALLÓ", evidencia });
    }
    console.error(`✗ Preparación: ${evidencia}`);
  } finally {
    if (tratamientoCreadoId) {
      await AppDataSource.getRepository(Tratamiento).delete(tratamientoCreadoId);
    }
    if (consultaPrueba) {
